...
public class Node {
	...
	public List<Node> getChildNodes() {
		...
		if ((yPos - 1) >= 0) {
			...
			childNodes.add(new Node(updatedLocations, xPos, yPos - 1, maxX, maxY,
					(directionFromParent == null ? "" : directionFromParent) + "U", this, heuristicType));
		}
		...
		if ((yPos + 1) < maxY) {
			int[][] updatedLocations = getChangedLocations(xPos, yPos + 1);
			childNodes.add(new Node(updatedLocations, xPos, yPos + 1, maxX, maxY,
					(directionFromParent == null ? "" : directionFromParent) + "D", this, heuristicType));
		}

		// left child
		if ((xPos - 1) >= 0) {
			int[][] updatedLocations = getChangedLocations(xPos - 1, yPos);
			childNodes.add(new Node(updatedLocations, xPos - 1, yPos, maxX, maxY,
					(directionFromParent == null ? "" : directionFromParent) + "L", this, heuristicType));
		}
		...
		if ((xPos + 1) < maxX) {
			int[][] updatedLocations = getChangedLocations(xPos + 1, yPos);
			childNodes.add(new Node(updatedLocations, xPos + 1, yPos, maxX, maxY,
					(directionFromParent == null ? "" : directionFromParent) + "R", this, heuristicType));
		}
	...
	private void setPathValues(Heuristic heuristicType) {
		if (this.directionFromParent == null) {
			this.costFromRoot = 0;
		} else {
			this.costFromRoot = directionFromParent.length();
		}
